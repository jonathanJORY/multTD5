...
void display()
    ...
    model = glm::rotate( model, glm::degrees( angle ), glm::vec3( 1.0f, 1.0f, 1.0f ) ) ;
    ...
    model = glm::translate( model, glm::vec3( decal, -decal, 0.0f ) );
    model = glm::rotate( model, glm::degrees( angle ), glm::vec3( 1.0f, 1.0f, 1.0f ) ) ;
    model = glm::scale(   model, glm::vec3( 8.70f) );
...
maillage initVAOs(shaderProg shader, std::string meshFile, std::string shadVert, std::string shadFrag)
{
...
check_gl_error();
    ...
    return maillage{shader, vaoids[0], nbtriangles, 0.0f, scale, inc, x, y, z};
}
...
shaderProg initShaders(std::string shadVert, std::string shadFrag)
{
   ...
   std::ifstream fs_ifs( MY_SHADER_PATH+ shadFrag );
    ...
    return shaderProg{progid, mvpid, projID, viewID, modelID};
}
